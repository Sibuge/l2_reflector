...
extern flexio_func_t flexio_my_rpc;
...
main(int argc, char **argv)
{
	...
	/* 初始化DPA设备 */
	/* 绑定了DPA上的事件处理函数 l2_reflector_device_event_handler */
	...
	/* 初始化DPA设备，即将完成从Host接受数据(sq和rq配置)的读取 */
	...
	while (!force_quit){
		...
		/* 一些LOG */
		ret = flexio_process_call(app_cfg.flexio_process, &flexio_my_rpc, &number, 0);
		if (ret != FLEXIO_STATUS_SUCCESS) {
			DOCA_LOG_ERR("Failed to call RPC function");
			result = DOCA_ERROR_DRIVER;
			goto rule_cleanup;
		}
		DOCA_LOG_INFO("The packets that have been received : %"PRIu64" ", number);
	}
